...
class EthereumWrapper():
    ...
    def __validate(self, config):
        ...
        self.validation.not_valid_contract(
            ethereum_account_id = config.get("eth_account"), 
            chain_id_in_config = config.get("chain_id"),
            ethereum_provider_url = config.get("provider"),
            parameter_gas_limit = config.get("gas_limit"),
            parameter_gas_price = config.get("gas_price"))
               
               
        provider = config.get("provider")
        
        if self._is_ropsten(provider):
            if not config.get("acc_pvt_key"):
                raise ContractFailed("Ethereum account private key is not set!!\
    ...
    def compile_source_file(self, file_path):
        ...
        source = files('avalon_sdk_ethereum').joinpath(file_path).read_text()
